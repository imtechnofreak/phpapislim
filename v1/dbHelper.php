...
class dbHelper {
    ...
    function insert($table, $columnsArray, $requiredColumnsArray) {
        ...
        try{
            ...
            $c = rtrim($c,', ');
            $v = rtrim($v,', ');
          //  echo "INSERT INTO $table($c) VALUES($v)";exit;
    ...
    { 
        try{
            ...
            if($affected_rows<=0){
                $response["status"] = "warning";
                $response["message"] = "No row";
            }else{
                $response["status"] = "success";
                $response["message"] = $affected_rows." row(s) in database";
            }
            $response["row"] = $affected_rows;
            $response["data"] = $data;
        }catch(PDOException $e){
            $response["status"] = "error";
    ...
    { 
        try{
            ...
            if($lastInsertId<=0){
                ...
                $response["status"] = "success";
                $response["message"] = " 1 row(s) in database";
            }
            $response["data"] = $lastInsertId;
        }catch(PDOException $e){
            $response["status"] = "error";
