		...
		if(strlen($phone_no)==10 && ctype_digit($phone_no) && ctype_digit($city))
		{
			...
			else
			{
				if(!empty($refer_by))
				{
					...
					if($query_login_refer_by['status']=="success")
					{
						...
						if($insert_user["status"] == "success")
						{
									...
									$coupon_referal_master = $db->customQuery("SELECT * FROM `coupon_referal_master` WHERE `status`='1' ORDER BY `coupon_referal_id` DESC LIMIT 0, 1 ");
									//echo $coupon_referal_master['data'][0]['coupan_type_for'];exit;
									if($coupon_referal_master['data'][0]['coupan_type_for']=="wallet_amount")
									{
										$wallet=$coupon_referal_master['data'][0]['amount_to_add'];
										$wallet_update = $db->update("user",array('wallet'=>$wallet),array('user_id'=>$user_id_in),array());
										$valid_date="Lifetime";
										$code='Not Available';
										$msg_instraller='Thanks for using referal code of your friend. Congrats '.$coupon_referal_master['data'][0]['coupon_referal_title'].' by Motorbabu!!';
										$msg_referer='Congrats! You will get '.$coupon_referal_master['data'][0]['coupon_referal_title'].' in your MotorBabu after complete vehicle serviced of referred friend.';
										$msg_n1='Congrats '.$coupon_referal_master['data'][0]['coupon_referal_title'].' by Motorbabu!!';
										$msg_n2='Congrats! You will get '.$coupon_referal_master['data'][0]['coupon_referal_title'].' by MotorBabu';
										$status=1;
										$status_referal=0;
									}
									else
									{
										//$coupon_referal_master = $db->select("coupon_referal_master","*",array('coupon_referal_id'=>'3', 'status'=>'1'));
										...
										$msg_instraller='Thanks for using referal code of your friend. Congrats you got the '.$coupon_referal_master['data'][0]['coupon_referal_title'].' by Motorbabu!!';
										$msg_referer='Thanks for referring Motorbabu to your friend. Congrats you got the '.$coupon_referal_master['data'][0]['coupon_referal_title'].' by Motorbabu!!';
										$msg_n1='You got the Amazing Offers by your referal code!!';
										$msg_n2='You got the Amazing Offers by your referal code!!';
										$status=0;
										$status_referal=0;
									}
									$user_id_in=$insert_user['data'];
									...
									$ab['message']= $msg_n1;
									$message= array('title'=>'Motorbabu','status'=>'referral_code','message'=>$msg_n1);
									AndroidNotification($device_token,$message);
									$notif_insert = $db->insert("app_notification",array('user_id'=>$user_id_in,'title'=>'Motorbabu','status'=>'referral_code','create_at'=>militime,'message'=>$msg_instraller),array());
									...
									$coupon_offer_user_2 = array( 'user_id'=>$query_login_refer_by['data'][0]['user_id'],
											...
											'status'=>$status_referal,
											'by_user_id'=>$user_id_in
											 );
									$insert_user = $db->insert("coupon_offer_user",$coupon_offer_user_2,array());
									
									$device_token= $query_login_refer_by['data'][0]['device_token'];
									$ab['message']= $msg_n2;
									$message= array('title'=>'Motorbabu','status'=>'referral_code','message'=>$msg_n2);
									AndroidNotification($device_token,$message);
									$notif_insert = $db->insert("app_notification",array('user_id'=>$user_id_reffer_by,'title'=>'Motorbabu','status'=>'referral_code','create_at'=>militime,'message'=>$msg_referer),array());
					...
					{
						$msg['message'] = "Invalid Referal code! Try again or you can leave blank!";
	...
	if(!empty($headers['secret_key']))
	{
		...
		if($check['status']=="true")
		{
			...
			if(isset($_GET['order_id']))
			{
				$user_id=$check['data'][0]['user_id'];
				global $db;
				//$center_data = $db->customQuery("SELECT center_id FROM service_order WHERE order_id = '$order_id'");
				//print_r($center_data);
				//$center_id=$center_data['data'][0];
				$order_transaction = $db->customQuery("SELECT transaction_amount, payment_method, payment_gateway_trasaction_id, transaction_date_time FROM service_order_transaction WHERE order_id = '$order_id' and transaction_status='1'");
				$arr_transaction = array();
				if($order_transaction['status'] == "success")
                {  
                    foreach($order_transaction['data'] as $transactionr_data)
                    {
                ...
                {  
                    foreach($order_services['data'] as $order_services1)
                    {
						...
						$subtotal_price=$subtotal_price+$order_services1['service_price'];
						$total_price=$subtotal_price;
                    }
                }
				if($order_id==1437)
				{
					$discount_amount=round(($subtotal_price*$discount_per)/100);
					$subtotal_price_1=$subtotal_price-$discount_amount;
					
					$tax_amount=round(($subtotal_price_1*$tax_per)/100);
					$total_price=$subtotal_price_1+$tax_amount;
					
					$discount_detail='10% discount on friend referral';
					$tax_detail='5% service tax according to Indian goverment';
				...
				$order_services['discount'] = $discount_amount;
				$order_services['tax'] = $tax_amount;
				$order_services['discount_detail'] = $discount_detail;
				$order_services['subtotal_price'] = $subtotal_price;
				$order_services['is_tax'] = $is_tax;
				$order_services['is_discount'] = $is_discount;
				$order_services['tax_detail'] = $tax_detail;
				$order_services['transaction'] = $arr_transaction;
	...
	if(!empty($headers['secret_key']))
	{
		...
		if($check['status']=="true")
		{
			...
			if(isset($_GET['city_id']))
			{
				...
				if($sort_type=="1")
				{
					$service_center = $db->customQuery("SELECT   service_center.center_name, service_center.center_id, service_center.center_code, service_center.center_address, service_center.rating, service_center.is_pickup_available, service_center.center_lat, service_center.center_lng, city.City, 
...
FROM service_center  LEFT JOIN city ON service_center.center_city = city.CityId WHERE service_center.center_lat!='0' and service_center.center_lng!='0' and service_center.center_city='$city_id' and service_center.status = '0' and service_center.center_type in (0,$vehicle_type)
ORDER BY distance");
				...
				{
					$service_center['message'] = "No more service centre found.";
					$service_center['list_center'] = "";
				}
				...
				if(ctype_digit($check['data'][0]['user_id']))
				{
					...
					if($data->status==1)
					{
						...
						if($insert_payment["status"] == "success")
						{
							...
							$transaction_query = $db->customQuery("SELECT transaction_amount FROM `service_order_transaction` WHERE `user_id`='$user_id' and order_id='$order_id' and transaction_status='1'");
							...
							$message= array('title'=>'Amount Piad','status'=>'order_book','message'=>$msg,'order_id'=>$order_id);
							AndroidNotification($device_token,$message);
							$notif_insert = $db->insert("app_notification",array('user_id'=>$user_id,'title'=>'Amount Piad','status'=>'order_book','create_at'=>militime,'message'=>$msg,'order_id'=>$order_id),array());
							//send notification to user start
							...
							if($center_user["status"] == "success")
							{
								...
								$notif_insert = $db->insert("app_notification",array('center_id'=>$center_id,'title'=>'Amount Piad','status'=>'order_book','create_at'=>militime,'message'=>$msg,'order_id'=>$order_id),array());
							}
							//send notification to center start
					...
					else if($data->status==2)
					{
						...
						$final_wallet_amount=$check['data'][0]['wallet'];
						...
						if($insert_payment["status"] == "success")
						{
							//send notification to user start
							$device_token=$check['data'][0]['device_token'];
							$msg="Transaction decline for your order No:".$order_id;
							$message= array('title'=>'Transaction decline','status'=>'order_book','message'=>$msg,'order_id'=>$order_id);
							AndroidNotification($device_token,$message);
							$notif_insert = $db->insert("app_notification",array('user_id'=>$user_id,'title'=>'Transaction decline','status'=>'order_book','create_at'=>militime,'message'=>$msg,'order_id'=>$order_id),array());
							//send notification to user start
							
							$insert_payment['message'] = "Sorry! transaction decline.";
							//$order_services['order_id'] = $order_id;
					...
					else
					{
						...
						if($insert_payment["status"] == "success")
						{
							//send notification to user start
							$device_token=$check['data'][0]['device_token'];
							$msg="Transaction failed for your order No:".$order_id;
							$message= array('title'=>'Transaction failed','status'=>'order_book','message'=>$msg,'order_id'=>$order_id);
							AndroidNotification($device_token,$message);
							$notif_insert = $db->insert("app_notification",array('user_id'=>$user_id,'title'=>'Transaction failed','status'=>'order_book','create_at'=>militime,'message'=>$msg,'order_id'=>$order_id),array());
							//send notification to user start
							
							$insert_payment['message'] = "Sorry! transaction failed.";
							//$order_services['order_id'] = $order_id;
				...
				if(ctype_digit($check['data'][0]['user_id']))
				{
					if($data->status==1)
					{
						...
						if($insert_payment["status"] == "success")
						{
							...
							//send notification to user start
							$device_token=$check['data'][0]['device_token'];
							$msg="Rs. ".$amount." added in wallet successfully! Your transaction id is :".$data->payment_id;
							$message= array('title'=>'Amount added','status'=>'wallet_added','message'=>$msg,'payment_gateway_trasaction_id'=>$data->payment_id);
							AndroidNotification($device_token,$message);
							$notif_insert = $db->insert("app_notification",array('user_id'=>$user_id,'title'=>'Amount added','status'=>'wallet_added','create_at'=>militime,'message'=>$msg),array());
							//send notification to user start
					...
					else
					{
						...
						if($insert_payment["status"] == "success")
						{
							...
							if($data->status==2)
							{
								$msg="Transaction decline! Amount not added in your wallet";
								$title="Transaction decline";
							}
							else
							{
								$msg="Transaction failed! Amount not added in your wallet";
								$title="Transaction failed";
							}
							$message= array('title'=>$title,'status'=>'wallet_added','message'=>$msg,'payment_gateway_trasaction_id'=>$data->payment_id);
							AndroidNotification($device_token,$message);
							$notif_insert = $db->insert("app_notification",array('user_id'=>$user_id,'title'=>$title,'status'=>'wallet_added','create_at'=>militime,'message'=>$msg),array());
							//send notification to user start
							
							$insert_payment['message'] = "Sorry! ".$title.".";
							//$order_services['order_id'] = $order_id;
...
$app->get('/transaction_list',function()use ($app){
	...
	if(!empty($headers['secret_key']))
	{
		...
		if($check['status']=="true")
		{
			...
			$transaction = $db->customQuery("SELECT * FROM service_order_transaction LEFT JOIN payment_gateway ON service_order_transaction.payment_gateway_id = payment_gateway.payment_gateway_id WHERE service_order_transaction.user_id='$user_id' and service_order_transaction.transaction_status='1' order by service_order_transaction_id desc");
