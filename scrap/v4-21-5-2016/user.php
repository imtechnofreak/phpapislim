			...
			if(!empty($json1))
			{
				...
				if(ctype_digit($check['data'][0]['user_id']))
				{
					...
					if($insert_payment["status"] == "success")
					{
						...
						$message= array('title'=>'Motorbabu','status'=>'wallet_added','message'=>$msg,'payment_gateway_trasaction_id'=>$data->payment_id);
						AndroidNotification($device_token,$message);
						$notif_insert = $db->insert("app_notification",array('user_id'=>$user_id,'title'=>'Motorbabu','status'=>'wallet_added','create_at'=>militime,'message'=>$msg),array());
						//send notification to user start
