...
$app->get('/Timeline',function(){
	...
	if(!empty($headers['secret_key']))
	{
		...
		if($check['status']=="true")
		{
			...
			if(ctype_digit($user_id))
			{
				$vehicle_query = $db->customQuery("SELECT service_order.order_id,service_order.center_id,service_order.vehicle_id,service_order.pickup_address_id,service_order.drop_address_id,service_order.create_at,service_order.update_at,service_order.final_amount,service_order.is_final_amount, model_variant.variant_name,model.model_name,make.make_name,service_center.center_name,service_center.center_address,service_center.rating,user_address.address,a2.address AS drop_address,user_vehicle.vehicle_reg,service_order.status,model.model_pic,make.make_logo,service_order.km,service_order.order_date,service_order.amount,service_order.service_time,service_order.is_pickup,service_order.center_rating FROM service_order LEFT JOIN user_vehicle ON service_order.vehicle_id=user_vehicle.vehicle_id LEFT JOIN model_variant ON user_vehicle.variant_id=model_variant.variant_id LEFT JOIN model ON model_variant.model_id=model.model_id LEFT JOIN make ON model.make_id=make.make_id  LEFT JOIN service_center ON service_order.center_id=service_center.center_id LEFT JOIN user_address ON service_order.pickup_address_id=user_address.address_id  LEFT JOIN user_address as a2 ON service_order.drop_address_id=a2.address_id WHERE service_order.user_id='$user_id'  ORDER BY service_order.order_id DESC");
				if($vehicle_query["status"] == "success")
				{
					$bb= array();
					$aa= array();
					foreach ($vehicle_query['data'] AS  $key) 
					{
						...
						if($sel_static_status['status']=="success")
						{
							...
							{
								$rating=$key['center_rating'];

								$arr0="";$arr1="";
								$arr2="";$arr3="";$arr4="";$arr5="";
								$check_status0=$db->select("order_status_master","msg_status_to_user,msg_user_short_code",array("status_id"=>0));
								$status="In Progress";
								$status_create_at="";
								$check_order0=$db->select("service_order_logs","*",array("order_id"=>$order_id,"status"=>0));
								if($check_order0['status']=="success")
								{
									$status="Complete";
									$status_create_at=date("d M Y", ($check_order0['data'][0]['create_at']/1000));
								}
								$arr0=array(
												"status"=>$status,
												"create_at"=>$status_create_at,
												"details"=>$check_status0['data'][0]['msg_status_to_user'],
												"status_name"=>$check_status0['data'][0]['msg_user_short_code']
											);
								...
								{	
									...
									$status="In Progress";
									$status_create_at="";
									$check_order2=$db->select("service_order_logs","*",array("order_id"=>$order_id,"status"=>3));//need to work for pickup
									...
									$arr= array($arr0,$arr1,$arr2,$arr3,$arr4,$arr5);
								}else
								{
									$arr= array($arr0,$arr1,$arr3,$arr4,$arr5);
								}
							...
							//get paid amount, esimate amount start
							$paid_amount=0;
							$remaining_amount=0;
							$advance_amount=0;
							$check_amount=$db->customQuery("SELECT transaction_amount FROM service_order_transaction WHERE order_id='$order_id' and transaction_type='1'");
							if($check_amount['status']=="success")
							{
								foreach ($check_amount['data'] as $amount) 
								{
									$paid_amount=$paid_amount+$amount['transaction_amount'];
								}
							}
							if($key['is_final_amount']==1)
							{
								$remaining_amount=$key['final_amount']-$paid_amount;
							}
							else
							{
								$advance_amount=$paid_amount;
							}
							//get paid amount, esimate amount end
							$aa[] =array(
								"order_id"=>$key['order_id'],
								"center_id"=>$key['center_id'],
								"center_name"=>$key['center_name'],
								...
								"pickup_address_id"=>$key['pickup_address_id'],
								"pickup_address"=>$key['address'],
								"drop_address_id"=>$key['drop_address_id'],
								"km"=>$key['km'],
								"drop_address"=>$key['drop_address'],
								"vehicle_image"=>$model_pic,
								"remaining_time"=>$remaining_time,
								"estimated"=>$key['amount'],
								"final_amount"=>$key['final_amount'],
								"remaining_amount"=>$remaining_amount,
								"advance_amount"=>$advance_amount,
								"is_final_amount"=>$key['is_final_amount'],
								//"rating"=>"4",
								"is_cancel"=>$is_cancel,
								"make_logo"=>$make_logo,
								"list_status"=>$arr,
								"order_date"=>date('d-M-Y', strtotime($key['order_date'])),
...
$app->get('/OrderDetail/:id',function($order_id)
{
	...
	if(!empty($headers['secret_key']))
	{
		...
		if($check['status']=="true")
		{
			...
			$vehicle_query = $db->customQuery("SELECT service_order.*,service_center.center_name,service_center.center_address,service_center.center_phone,service_center.center_mobile,service_center.rating,service_center.is_billing_enable,user_address.address,a2.address AS drop_address,order_timeslot_master.slot_name,model_variant.variant_name,model.model_name,make.make_name,user_vehicle.vehicle_reg,service_order.status,model.model_pic,make.make_logo FROM service_order LEFT JOIN user_vehicle ON service_order.vehicle_id=user_vehicle.vehicle_id LEFT JOIN model_variant ON user_vehicle.variant_id=model_variant.variant_id LEFT JOIN model ON model_variant.model_id=model.model_id LEFT JOIN make ON model.make_id=make.make_id LEFT JOIN service_center ON service_order.center_id=service_center.center_id LEFT JOIN user_address ON service_order.pickup_address_id=user_address.address_id  LEFT JOIN user_address as a2 ON service_order.drop_address_id=a2.address_id LEFT JOIN order_timeslot_master ON service_order.order_time=order_timeslot_master.slot_id WHERE service_order.order_id='$order_id'");
			if($vehicle_query["status"] == "success")
			{
				...
				if($vehicle_query['data'][0]['model_pic'])
				{
					$model_pic=base_url.$vehicle_query['data'][0]['model_pic'];
				}else
				{
					$model_pic="";
				}

				if($vehicle_query['data'][0]['make_logo'])
				{
					$make_logo=base_url."/images/make/640x960/images/".$vehicle_query['data'][0]['make_logo'];
				}else
				{
					$make_logo="";
				}
						...
						if($sel_static_status['status']=="success")
						{
							if($vehicle_query['data'][0]['status']=="2" || $vehicle_query['data'][0]['status']=="4" || $vehicle_query['data'][0]['status']=="8")
							{
								$check_status=$db->customQuery("SELECT service_order_logs.*,order_status_master.msg_status_to_user,order_status_master.msg_user_short_code FROM service_order_logs LEFT JOIN order_status_master ON service_order_logs.status=order_status_master.status_id WHERE order_id='$order_id'");
								if($check_status['status']=="success")
								{
									foreach ($check_status['data'] as $check_status1) 
									{
										$arr[]=array(
												...
												"status"=>"Complete",
												"create_at"=>date("d M Y", ($check_status1['create_at']/1000)),
												"details"=>$check_status1['msg_status_to_user']." at ".date("d-M h:i A", ($check_status1['create_at']/1000)),
												"status_name"=>$check_status1['msg_user_short_code']
											);
									}
								}
							...
							}else 
							{
								$arr0="";$arr1="";
								$arr2="";$arr3="";$arr4="";$arr5="";
								$date_time_at0='';$date_time_at1='';$date_time_at2='';$date_time_at3='';$date_time_at4='';$date_time_at5='';$date_time_at6='';
								...
								if($check_order0['status']=="success")
								{
									$status="Complete";
									$status_create_at=date("d M Y", ($check_order0['data'][0]['create_at']/1000));
									$date_time_at0=" at ".date("d-M h:i A", ($check_order0['data'][0]['create_at']/1000));
								}
								$arr0=array(
												"status"=>$status,
												"create_at"=>$status_create_at,
												"details"=>$check_status0['data'][0]['msg_status_to_user'].$date_time_at0,
												"status_name"=>$check_status0['data'][0]['msg_user_short_code']
											);
								...
								if($check_order1['status']=="success")
								{
									$status="Complete";
									$status_create_at=date("d M Y", ($check_order1['data'][0]['create_at']/1000));
									$date_time_at1=" at ".date("d-M h:i A", ($check_order1['data'][0]['create_at']/1000));
								}
								$arr1=array(
												"status"=>$status,
												"create_at"=>$status_create_at,
												"details"=>$check_status1['data'][0]['msg_status_to_user'].$date_time_at1,
												"status_name"=>$check_status1['data'][0]['msg_user_short_code']
											);
								...
								if($check_order3['status']=="success")
								{
									$status="Complete";
									$status_create_at=date("d M Y", ($check_order3['data'][0]['create_at']/1000));
									$date_time_at3=" at ".date("d-M h:i A", ($check_order3['data'][0]['create_at']/1000));
								}
								$arr3=array(
												"status"=>$status,
												"create_at"=>$status_create_at,
												"details"=>$check_status3['data'][0]['msg_status_to_user'].$date_time_at3,
												"status_name"=>$check_status3['data'][0]['msg_user_short_code']
											);
								...
								if($check_order4['status']=="success")
								{
									$status="Complete";
									$status_create_at=date("d M Y", ($check_order4['data'][0]['create_at']/1000));
									$date_time_at4=" at ".date("d-M h:i A", ($check_order4['data'][0]['create_at']/1000));
								}
								$arr4=array(
												"status"=>$status,
												"create_at"=>$status_create_at,
												"details"=>$check_status4['data'][0]['msg_status_to_user'].$date_time_at4,
												"status_name"=>$check_status4['data'][0]['msg_user_short_code']
											);
								$check_status5=$db->select("order_status_master","msg_status_to_user,msg_user_short_code",array("status_id"=>7));
								$status="In Progress";
								...
								if($check_order5['status']=="success")
								{
									$status="Complete";
									$status_create_at=date("d M Y", ($check_order5['data'][0]['create_at']/1000));
									$date_time_at5=" at ".date("d-M h:i A", ($check_order5['data'][0]['create_at']/1000));
								}
								$arr5=array(
												"status"=>$status,
												"create_at"=>$status_create_at,
												"details"=>$check_status5['data'][0]['msg_status_to_user'].$date_time_at5,
												"status_name"=>$check_status5['data'][0]['msg_user_short_code']
											);
								if($vehicle_query['data'][0]['is_pickup']=="1")
								{	
									$check_status2=$db->select("order_status_master","msg_status_to_user,msg_user_short_code",array("status_id"=>5));
									$status="In Progress";
									$status_create_at="";
									...
									if($check_order2['status']=="success")
									{
										$status="Complete";
										$status_create_at=date("d M Y", ($check_order2['data'][0]['create_at']/1000));
										$date_time_at2=" at ".date("d-M h:i A", ($check_order2['data'][0]['create_at']/1000));
									}
									$arr2=array(
													"status"=>$status,
													"create_at"=>$status_create_at,
													"details"=>$check_status2['data'][0]['msg_status_to_user'].$date_time_at2,
													"status_name"=>$check_status2['data'][0]['msg_user_short_code']
												);
									$arr= array($arr0,$arr1,$arr2,$arr3,$arr4,$arr5);
								}else
		    	...
		    	$wallet_amount =  $wallet_query['data'][0]['wallet'];
							...
							if($vehicle_query['data'][0]['is_final_amount']==1)
							{
								$remaining_amount=$vehicle_query['data'][0]['final_amount']-$paid_amount;
							}
							else
							{
								$advance_amount=$paid_amount;
				...
				$aa = array(
						"order_id"=>$vehicle_query['data'][0]['order_id'],
						"center_id"=>$vehicle_query['data'][0]['center_id'],
						"order_date"=>date('d-M-Y', strtotime($vehicle_query['data'][0]['order_date'])),
						"order_time"=>$vehicle_query['data'][0]['slot_name'],
						"km"=>$vehicle_query['data'][0]['km'],
						"services_arr"=>$v4,
						"services"=>$servvv,
						...
						"is_billing_enable"=>$vehicle_query['data'][0]['is_billing_enable'],
						"remaining_time"=>$vehicle_query['data'][0]['service_time'],
						"estimated"=>$vehicle_query['data'][0]['amount'],
						"remaining_amount"=>$remaining_amount,
						"advance_amount"=>$advance_amount,
						"final_amount"=>$vehicle_query['data'][0]['final_amount'],
						"paid_amount"=>$paid_amount,
						"is_final_amount"=>$vehicle_query['data'][0]['is_final_amount'],
						"order_payment_status"=>$vehicle_query['data'][0]['order_payment_status'],
						"pickup_address"=>$vehicle_query['data'][0]['address'],
						"drop_address"=>$vehicle_query['data'][0]['drop_address'],
						"center_phone"=>$vehicle_query['data'][0]['center_phone'],
						"center_mobile"=>$vehicle_query['data'][0]['center_mobile'],
						"list_status"=>$arr,
						"vehicle_id"=>$vehicle_query['data'][0]['vehicle_id'],
						"vehicle_reg"=>$vehicle_query['data'][0]['vehicle_reg'],
						"vehicle"=>$vehicle_query['data'][0]['make_name']." ".$vehicle_query['data'][0]['model_name']." ".$vehicle_query['data'][0]['variant_name'],
						"vehicle_image"=>$model_pic,
						"make_name"=>$vehicle_query['data'][0]['make_name'],
						"model_name"=>$vehicle_query['data'][0]['model_name'],
						"make_logo"=>$make_logo,
						"variant_name"=>$vehicle_query['data'][0]['variant_name'],
...
$app->get('/coupon',function()use ($app){
	...
	if(!empty($headers['secret_key']))
	{
		...
		if($check['status']=="true")
		{
			$coupon_code=$app->request()->params('coupon_code');
			global $db;
			
			$version_query = $db->select("coupon","coupon_code,amount,coupon_type,detail",array('coupon_code'=>$coupon_code,"status"=>0));
			if($version_query["status"] == "success")
			{
				$aa=array(
						"coupon_code"=>$version_query['data'][0]['coupon_code'],
						"amount"=>$version_query['data'][0]['amount'],
						"coupon_type"=>$version_query['data'][0]['coupon_type'],

					);
				$version_query['message'] = $version_query['data'][0]['detail'];
				unset($version_query['status']);
				$version_query['data']=$aa;
				echoResponse(200,$version_query);
				
			}else
			{
				$version_query['message']= "Invalid Code";
				echoResponse(200,$version_query);
			}
...
$app->put('/Feedback',function() use ($app)
{
	$headers = apache_request_headers();
	if(!empty($headers['secret_key']))
	{
		...
		if($check['status']=="true")
		{
			...
			{
				global $db;
				$check_status= $db->customQuery("SELECT * FROM `service_order` where order_id='$order_id' AND user_id='$user_id'");
				if($check_status['status']=="success")
				{
					$update_order_status = $db->update("service_order",array("center_rating"=>$rating,"center_comment"=>$comment,"update_at"=>militime),array("order_id"=>$order_id),array());
