...
/*$app->get('/vehicleById/:id',function($vehicle_id) use ($app){
	$headers = apache_request_headers();
	if(!empty($headers['secret_key']))
	{
		$check = token_auth($headers['secret_key']);
		if($check['status']=="true")
		{
			...
			if(ctype_digit($user_id))
			{
				$vehicle_query = $db->customQuery("SELECT vehicle.vehicle_id,vehicle.variant_id,vehicle.vehicle_reg,model_variant.variant_name,model.model_name,make.make_name,make.make_type FROM vehicle left join model_variant on vehicle.variant_id=model_variant.variant_id left join model on model_variant.model_id=model.model_id left join make on model.make_id=make.make_id WHERE vehicle.vehicle_id='$vehicle_id'");
				if($vehicle_query["status"] == "success")
				{
					$vehicle_query['message'] = "successfully";
					unset($vehicle_query['status']);
		...
		if($check['status']=="true")
		{
			...
			if(!empty($vehicle_id))
			{
				...
				if(ctype_digit($check['data'][0]['user_id']))
				{
					...
					if($vehicle_query["status"] == "success")
					{
						$update_vehicle = $db->update("vehicle", $upd_data, array('user_id'=>$user_id,'vehicle_id'=>$vehicle_id),array());
						if($update_vehicle["status"] == "success")
						{
							$update_vehicle['message'] ="vehicle updated successfully";

							$sel_det = $db->select("vehicle","*",array('user_id'=>$user_id,'vehicle_id'=>$vehicle_id));
...
$app->get('/search',function(){
	...
	if(!empty($headers['secret_key']))
	{
		...
		if($check['status']=="true")
		{
			...
			if(ctype_digit($user_id))
			{
				$vehicle_query = $db->customQuery("SELECT model_variant.variant_id,model_variant.variant_name,model.model_name,model.model_pic,make.make_name,make.make_logo FROM model_variant LEFT JOIN model ON model_variant.model_id=model.model_id LEFT JOIN make ON make.make_id=model.make_id WHERE variant_status='0'");
				if($vehicle_query["status"] == "success")
				{
					$vehicle_query['message'] = "successfully";
					unset($vehicle_query['status']);
					...
					foreach ($vehicle_query['data'] as $key) 
					{
						if($key['model_pic'])
						{
							$model_pic=base_url."/dashboard/".$key['model_pic'];
						}else
						{
							$model_pic="";
						}
						...
						"make_logo"=>base_url."/dashboard/images/make/640x960/images/".$key['make_logo'],
...
$app->get('/History',function(){
	...
	if(!empty($headers['secret_key']))
	{
		...
		if($check['status']=="true")
		{
			...
			if(ctype_digit($user_id))
			{
				$vehicle_query = $db->customQuery("SELECT vehicle.*,model_variant.variant_name,model.model_name,model.model_pic,make.make_name,make.make_type,make.make_logo FROM vehicle  LEFT JOIN model_variant ON vehicle.variant_id=model_variant.variant_id LEFT JOIN model ON model_variant.model_id=model.model_id LEFT JOIN make ON model.make_id=make.make_id WHERE vehicle.user_id='$user_id' ");
				if($vehicle_query["status"] == "success")
				{
					foreach ($vehicle_query['data'] AS  $key) 
					{
						...
						$vehicle_order = $db->customQuery("SELECT service_order.order_id,service_order.center_id,service_order.vehicle_id,service_order.pickup_address_id,service_order.drop_address_id,service_order.services,service_order.create_at,service_order.update_at,service_center.center_name,service_center.center_address,address.street,address.location,address.zip_code,address.city,a2.street AS drop_street,a2.location AS drop_location,a2.zip_code AS drop_zip_code,a2.city AS drop_city FROM service_order LEFT JOIN service_center ON service_order.center_id=service_center.center_id LEFT JOIN address ON service_order.pickup_address_id=address.address_id  LEFT JOIN address as a2 ON service_order.drop_address_id=a2.address_id WHERE service_order.vehicle_id='".$key['vehicle_id']."'");
