	...
	if(!empty($headers['secret_key']))
	{
		...
		if($check['status']=="true")
		{
			...
			if(ctype_digit($user_id))
			{
				...
				$vehicle_query = $db->customQuery("SELECT review.*,user.name,user.email,user.profile_pic FROM review INNER JOIN user ON review.user_id=user.user_id WHERE review.center_id=$id  ORDER BY review.review_id DESC LIMIT 2");
