	...
	if(!empty($headers['secret_key']))
	{
		...
		if($check['status']=="true")
		{
			...
			if(ctype_digit($user_id))
			{
				...
				$last_ser = $db->customQuery("SELECT * FROM service_order WHERE service_order.user_id='$user_id' AND service_order.vehicle_id='$vehicle_id' AND service_order.status!='8' AND service_order.status!='2' ORDER BY service_order.order_id DESC LIMIT 1");
